...
class Day09(rawInput: List[String]):
    ...
    def solvePart2(input: InputType, target: Long) =
...
object Day09:
    ...
    def isValid(numList: List[Long], target: Long) =
        @tailrec
    ...
    def subarraySum(numList: List[Long], target: Long) =
        @tailrec
        def recur(from: Int, to: Int, total: Long): (Int, Int) =
            total match
                ...
                case _                             => (from, to)
        recur(0, 2, numList(0) + numList(1))
    end subarraySum
end Day09

/*--------- Block to test this file on IDEs, comment this line with `//` to enable.
@main def run_2020_09 =
